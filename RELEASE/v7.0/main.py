...
display_frame1 = params.get_frames(frame=1)
display_frame2 = params.get_frames(frame=1)

model = ARModel.wiggle(model, state[0])
query = model.get_points(all_points=True)
image1, hsv, edges, dt = create_image(query, w*sf,h*sf, np.copy(display_frame1[0]))
model = ARModel.wiggle(model, state[0], inverse=True)

display = add_points(query, np.copy(display_frame1[0]))

cv2.imshow("1", display)

model = ARModel.wiggle(model, state[1])
query = model.get_points(all_points=True)
image2, hsv, edges, dt = create_image(query, w*sf,h*sf, np.copy(display_frame1[0]))
model = ARModel.wiggle(model, state[1], inverse=True)

display = add_points(query, np.copy(display_frame2[0]))

grey1 = cv2.cvtColor(np.copy(image1.astype(np.float32)), cv2.COLOR_BGR2GRAY)
grey2 = cv2.cvtColor(np.copy(image2.astype(np.float32)), cv2.COLOR_BGR2GRAY)

flow = cv2.calcOpticalFlowFarneback(grey1,grey2, None, 0.5, 3, 15, 3, 5, 1.2, 0)
