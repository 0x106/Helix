...
class parameters(object):
	...
	def get_frames(self, update_iter=0, frame=-1):
		if frame == -1:
			[self.update(frame=frame) for i in range(update_iter)]
		else:
			self.update(frame=frame)
		return np.copy(self.image), np.copy(self.hsv), np.copy(self.dt), np.copy(self.edges)
